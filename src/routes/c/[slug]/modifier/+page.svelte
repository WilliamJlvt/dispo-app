...
<div class="max-w-lg mx-auto px-4 py-10">
  <div class="mb-6">
    <a href="/c/{c.id}" class="inline-flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-950">
  ...
  <div class="mb-8">
    ...
    <p class="text-sm text-zinc-500 mt-1">Les réponses existantes sont conservées.</p>
  </div>
  ...
  <form method="POST" class="space-y-5">
    ...
    <label class="flex items-center gap-2.5 cursor-pointer">
      <input
        ...
        class="w-4 h-4 rounded border-zinc-300 text-zinc-900"
      />
    ...
    <div class="flex gap-3 pt-1">
      <button
        type="submit"
        class="flex-1 h-9 bg-zinc-900 hover:bg-zinc-800 text-white text-sm font-medium rounded-md shadow-sm"
      >
        Enregistrer
      </button>
      <a
        href="/c/{c.id}"
        class="flex-1 h-9 flex items-center justify-center border border-zinc-200 hover:bg-zinc-50 text-zinc-700 text-sm font-medium rounded-md"
      >
        Annuler
      </a>
    </div>
